...
| Full Disk Access / DB 可访问性诊断 | 已实现（强化） | `doctor` 返回固定 `components` + command-level `capabilities`，overall 仅为 `ready/partial/unavailable`；单个 store/config/worker 失败不会把无关能力判死 |
