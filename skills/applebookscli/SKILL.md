...
- Ordinary reads are bounded semantic output. When `truncatedFields` is present, treat those values as valid but incomplete presentation text; do not invent the missing tail. If the user explicitly needs original full text/CFI, use archival export instead of repeatedly widening an ordinary query.
