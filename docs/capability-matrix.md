...
| Books.app lifecycle | 已实现（强化） | 非变异前置检查先完成；CLI 默认 live mutation 按 domain 管理 Books，单侧 DB override 的对应 writer 使用 detached lifecycle；公开 `AppleBooksCore` 仍允许调用方通过既有 `manageBooksApplication` 或显式双-domain initializer 请求生命周期管理，automatic cloud rail 是否可用仍由 canonical live store 决定。受管理 mutation 捕获 `closed / background / frontmost`；annotation 与 collection single-record acknowledgement 都并入 coordinator 的 post-COMMIT lifecycle：临时启动使用 non-activating launch，collection 可先 recycle `bookdatastored`；ack 后 background/frontmost 恢复原状态；原 closed 只清理本次 sync 明确标记为自己临时启动、且收尾时仍未被用户切到前台的 Books。恢复失败仍为 committed warning；BKLibrary restore 保留既有 running/closed contract |
