...
struct CollectionFacadeTests {
    ...
    @Test
    func automaticCollectionSyncAcknowledgesWithoutExplicitFlagAndRestoresClosedBooks() throws {
        let fixture = try fixture()
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        var events: [String] = []
        var running = false
        ...
        let books = try AppleBooks(
            ...
            automaticCollectionSync: true
        )

        let result = try books.createCollection(title: "Automatic")
    ...
    @Test
    func injectedFacadeDoesNotAutomaticallySyncCollectionsByDefault() throws {
        let fixture = try fixture()
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        var events: [String] = []
