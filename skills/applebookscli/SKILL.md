...
- `annotations update-note <uuid>` reads the complete replacement note from stdin; read the current note first when the user wants to append. Use `--clear` only to clear the note, and do not send a note body with `--clear`. `annotations delete` soft-deletes the annotation.
