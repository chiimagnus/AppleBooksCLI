...
struct CapabilityParityTests {
    @Test
    func coreVerifiedMatrixRowsHaveRealImplementationAndExecutableTestAnchors() throws {
        ...
        #expect(
            anchorKeys == coreVerified,
            "core anchor mismatch; missing=\(coreVerified.subtracting(anchorKeys).sorted()) extra=\(anchorKeys.subtracting(coreVerified).sorted())"
        )

        for capability in anchorKeys.sorted() {
            let value = try #require(object[capability] as? [String: Any])
            #expect(
                Set(value.keys) == ["implementationPaths", "testPaths"],
            ...
            for path in implementationPaths {
                ...
                #expect(path.hasPrefix("Sources/AppleBooksCLI/") == false, "P6 core anchor must not point at CLI reachability")
