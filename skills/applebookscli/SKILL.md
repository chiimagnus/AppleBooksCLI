...
4. When a result returns `nextCursor`, continue the same query with `--cursor <nextCursor>` and pass the token unchanged. Growing reading-state queries use this cursor contract (default 20, max 100); do not use `--offset`.
