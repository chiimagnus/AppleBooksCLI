...
- canonical live annotation 与 collection mutation 默认等待当前 Mac 的 CloudKit acknowledgement。mutation 命令不再提供逐条 `--sync`；只有显式恢复/flush pending cloud changes 时才使用根 `sync`。
- 自动 acknowledgement 或根 `sync` 成功都只证明当前 Mac 上的 Apple Books cloud records 获得 CloudKit acknowledgement；没有第二台设备的证据时，不声称其它设备已经显示。
